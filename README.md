...
Для работы системы регистрации и восстановления пароля необходимо настроить SMTP.
...
2. Откройте `.env` и заполните настройки SMTP:
   ...
   **Важно:** Для Gmail нужно использовать "Пароль приложения", а не обычный пароль:
...
Если SMTP не настроен, система будет работать в режиме разработки:
- Email не будет отправляться
- Код подтверждения будет выводиться в консоль сервера
- Регистрация будет работать нормально

### Пример вывода кода в консоли:
...
Тема: Подтверждение регистрации на SST Site
Текст: Ваш код подтверждения: 123456

🔐 КОД ПОДТВЕРЖДЕНИЯ: 123456
```

--- - Сайт для терміналів самообслуговування
